...
import com.hospital.model.login.dao.RootDao;
 ...
 */
@Service
public class RootServiceImpl extends ServiceImpl<RootDao, UserEntity> implements RootIService {
    ...
    public void changeStatus(RootForm rootForm) {
        ...
        rootDao.updateById(userEntity);
    }

    @Override
    public boolean deleted(String id) {
        int i = rootDao.deleteById(id);
        if (i == 1) {
            return true;
        }
        return false;
    ...
    public boolean insert(RootForm rootForm) {
        ...
        QueryWrapper<UserEntity> query = new QueryWrapper<>();
        query.eq("root_id", rootId);
        Long count = rootDao.selectCount(query);
        if (count == 0) {
            //不重复则如插入
            UserEntity userEntity = new UserEntity();
    ...
    public boolean edit(RootForm rootForm) {
        ...
        userEntity.setPassword(rootForm.getPassword());
        userEntity.setStatus(rootForm.isStatus());
        int count = rootDao.updateById(userEntity);
        if (count == 0) {
            return false;
        }
        return true;
